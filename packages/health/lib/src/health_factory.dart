...
class HealthFactory {
  ...
  Future<bool> isHealthConnectAvailable({install = false}) async {
    ...
    final bool? isAvailable =
        await _channel.invokeMethod('isHealthConnectAvailable', args);
      ...
      if (success.length > 0) {
        if (type == HealthDataType.WEIGHT) {
          ...
          return success.map<HealthConnectHydration>((e) {
            return HealthConnectHydration.fromJson(
                e as Map<dynamic, dynamic>, type);
          }).toList();
        } else if (type == HealthDataType.STEPS) {
          return success.map<HealthConnectSteps>((e) {
            return HealthConnectSteps.fromJson(
                e as Map<dynamic, dynamic>, type);
          }).toList();
        }
      }
